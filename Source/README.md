...
1.0.12 产物位于 `dist/CalendarCountdown-1.0.12-macos-universal.dmg`，内含 arm64 和 x86_64 可执行文件。当前构建采用 ad-hoc 签名，不等同于经过 Developer ID 签名和 Apple 公证的发行包。验证和安装只启动 `/Applications/知行.app`，不要打开 DerivedData 里的 `CalendarCountdown.app`。
