# 知行 1.0.18（macOS Universal）

## 安装 App

1. 打开 `CalendarCountdown-1.0.18-macos-universal.dmg`。
...
5. 替换已安装的“知行.app”不会清空任务、使命、打卡或日历数据；数据保存在 App Group 容器和 Apple 日历中，不在应用包内。
...
```bash
/Volumes/知行\ 1.0.18/calcount version
/Volumes/知行\ 1.0.18/calcount auth
/Volumes/知行\ 1.0.18/calcount import "/Volumes/知行 1.0.18/导入格式示例.json" --dry-run
/Volumes/知行\ 1.0.18/calcount tracking export --output ~/Desktop/important-days.json
```
